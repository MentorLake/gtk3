...
public static class PangoFontDescriptionExtensions
{
	...
	{
		if (desc.IsInvalid || desc.IsClosed) throw new Exception("Invalid or closed handle (PangoFontDescription)");
	...
	{
		if (desc1.IsInvalid || desc1.IsClosed) throw new Exception("Invalid or closed handle (PangoFontDescription)");
