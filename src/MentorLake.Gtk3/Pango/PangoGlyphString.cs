...
public static class PangoGlyphStringExtensions
{
	...
	{
		if (@string.IsInvalid || @string.IsClosed) throw new Exception("Invalid or closed handle (PangoGlyphString)");
	...
	{
		if (glyphs.IsInvalid || glyphs.IsClosed) throw new Exception("Invalid or closed handle (PangoGlyphString)");
