...
public static class PangoLayoutLineExtensions
{
	...
	{
		if (line.IsInvalid || line.IsClosed) throw new Exception("Invalid or closed handle (PangoLayoutLine)");
	...
	{
		if (layout_line.IsInvalid || layout_line.IsClosed) throw new Exception("Invalid or closed handle (PangoLayoutLine)");
