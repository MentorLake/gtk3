...
public static class PangoGlyphItemExtensions
{
	...
	{
		if (glyph_item.IsInvalid || glyph_item.IsClosed) throw new Exception("Invalid or closed handle (PangoGlyphItem)");
	...
	{
		if (orig.IsInvalid || orig.IsClosed) throw new Exception("Invalid or closed handle (PangoGlyphItem)");
