...
public static class GIOStreamHandleExtensions
{
	...
	{
		if (stream.IsInvalid || stream.IsClosed) throw new Exception("Invalid or closed handle (GIOStreamHandle)");
	...
	{
		if (stream1.IsInvalid || stream1.IsClosed) throw new Exception("Invalid or closed handle (GIOStreamHandle)");
