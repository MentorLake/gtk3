...
public static class graphene_frustum_tExtensions
{
	...
	{
		if (f.IsInvalid || f.IsClosed) throw new Exception("Invalid or closed handle (graphene_frustum_t)");
	...
	{
		if (a.IsInvalid || a.IsClosed) throw new Exception("Invalid or closed handle (graphene_frustum_t)");
