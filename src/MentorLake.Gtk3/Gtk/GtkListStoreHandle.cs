...
public static class GtkListStoreHandleExtensions
{
	...
	{
		if (list_store.IsInvalid || list_store.IsClosed) throw new Exception("Invalid or closed handle (GtkListStoreHandle)");
	...
	{
		if (store.IsInvalid || store.IsClosed) throw new Exception("Invalid or closed handle (GtkListStoreHandle)");
