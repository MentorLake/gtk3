...
public static class PangoLayoutHandleExtensions
{
	...
	{
		if (layout.IsInvalid || layout.IsClosed) throw new Exception("Invalid or closed handle (PangoLayoutHandle)");
	...
	{
		if (src.IsInvalid || src.IsClosed) throw new Exception("Invalid or closed handle (PangoLayoutHandle)");
